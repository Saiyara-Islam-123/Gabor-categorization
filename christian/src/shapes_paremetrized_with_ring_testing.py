...
def _shape_boundary(
    # common
    R: float,                  # interpreted by 'radius_profile'
    k1: int, k2: int,
    num_samples: int = 512,
    rot_rad: float = 0.0,
    mode: str = "phase",       # "phase" | "mode" | "indep_phase"
    radius_profile: str = "normalized",  # NEW: "normalized" | "absolute"
    # inputs for mode="phase" (shared phase via x,y)
    x: float = None, y: float = None,
    a1: float = 0.0, a2: float = 0.0,
    # inputs for mode="mode" (x drives k1, y drives k2)
    alpha_x: float = 0.0, beta_y: float = 0.0,
    # inputs for mode="indep_phase" (direct phases)
    phi1: float = None, phi2: float = None
):
    theta = np.linspace(0, 2*np.pi, num_samples, endpoint=False)
    theta_rot = theta + rot_rad

    # start with radius depending on profile
    if radius_profile == "normalized":
        r = np.ones_like(theta_rot, dtype=np.float64) * R
        add = lambda term: term                               # will be multiplied by R at end (existing behavior)
    elif radius_profile == "absolute":
        r = np.ones_like(theta_rot, dtype=np.float64) * R     # R is the *base* radius (unit before pixel scale)
        add = lambda term: term                                # add directly in the same units as R
    else:
        raise ValueError("radius_profile must be 'normalized' or 'absolute'")

    if mode == "phase":
        assert x is not None and y is not None
        # shared phase terms (same as before)
        if a1 != 0.0:
            term = a1 * (x * np.cos(k1 * theta_rot) + y * np.sin(k1 * theta_rot))
            if radius_profile == "normalized": r += R * term
            else:                               r +=     term
        if a2 != 0.0:
            term = a2 * (x * np.sin(k2 * theta_rot) - y * np.cos(k2 * theta_rot))
            if radius_profile == "normalized": r += R * term
            else:                               r +=     term
        ...
        assert x is not None and y is not None
        if alpha_x != 0.0:
            term = alpha_x * x * np.cos(k1 * theta_rot)
            if radius_profile == "normalized": r += R * term
            else:                               r +=     term
        if beta_y != 0.0:
            term = beta_y  * y * np.cos(k2 * theta_rot)
            if radius_profile == "normalized": r += R * term
            else:                               r +=     term

    elif mode == "indep_phase":
        assert phi1 is not None and phi2 is not None
        # **wrapped cosine(s)** when radius_profile == "absolute":
        # r(θ) = R + a1 cos(k1(θ-φ1)) + a2 cos(k2(θ-φ2))
        if a1 != 0.0:
            term = a1 * np.cos(k1 * (theta_rot - phi1))
            if radius_profile == "normalized": r += R * term
            else:                               r +=     term
        if a2 != 0.0:
            term = a2 * np.cos(k2 * (theta_rot - phi2))
            if radius_profile == "normalized": r += R * term
            else:                               r +=     term
    else:
        raise ValueError("mode must be 'phase', 'mode', or 'indep_phase'")
...
def _render_shape_image(
    ...
    radius_profile: str = "normalized",
    amplitude_units: str = "relative",  # NEW: "relative" | "pixels",
    amp_gain: float = 0.0        # <— NEW: 0.0 means no change
):
    """
    amplitude_units:
      - "relative": a1,a2 are unitless fractions (scaled by ring_radius_norm)  [OLD behavior]
      - "pixels":   a1,a2 are already in pixels (do NOT scale them by ring_radius_norm)
    """

    if radius_profile == "absolute" and amplitude_units == "pixels":
        R_px = float(ring_radius_norm)
        a1_px = float(a1) * (1.0 + amp_gain)  # ← APPLY GAIN HERE
        a2_px = float(a2) * (1.0 + amp_gain)  # ← AND HERE
        xs, ys = _shape_boundary(
            R=R_px, k1=k1, k2=k2, num_samples=512, rot_rad=global_rot_rad,
            mode=mode, radius_profile="absolute",
            x=x, y=y,
            a1=a1_px, a2=a2_px,
        ...
        # OLD behavior (normalized + relative) and also absolute+relative:
        # construct in unit space and scale at raster time by ring_radius_norm
        # When building in unit space (the common path), just scale the harmonic amplitudes:
        xs, ys = _shape_boundary(
            R=1.0, k1=k1, k2=k2, num_samples=512, rot_rad=global_rot_rad,
            mode=mode, radius_profile=radius_profile,
            x=x, y=y,
            a1=a1 * (1.0 + amp_gain),  # <— apply gain
            a2=a2 * (1.0 + amp_gain),  # <— apply gain
            alpha_x=alpha_x, beta_y=beta_y,
            phi1=phi1, phi2=phi2
        )
...
class ShapeDeformDataset(Dataset):
    def __init__(self, nA, nB, m_arcs_per_class=2, gap_frac=0.2, phase_deg=0.0,
                 ...
                 a1=0.1, a2=0.1,
                 mode="indep_phase",
                 radius_profile="absolute", amplitude_units="relative",
        ...
        for i in range(nA + nB):
            ...
            lab = labels[i]  # 0 or 1
            amp_gain = 0.0 if lab == 0 else self.difficulty_sharp  # Class 1 gets a boost
...
def load_shape_deform_data(
    ...
    a1=0.1, a2=0.1,
    mode="indep_phase",
    radius_profile="absolute",   # <- NEW
    amplitude_units="relative",  # default
    alpha_x=0.1, beta_y=0.1,
...
def main():
    ...
    amp_ring_radius = 20  # smaller = subtler deformation
    amp_scale_a1 = 0.4
    amp_scale_a2 = 0.3
    ...
    trainloader, valloader, testloader, ds = load_shape_deform_data(
        ...
        amplitude_units="pixels",  # default
        alpha_x=0.1, beta_y=0.1,
        intensity=1.0, bg=0.0, norm="max", global_rot="none",
        amp_mode=amp_mode, amp_class_coupling=amp_class_coupling,
        difficulty_sharp=difficulty_sharp,
